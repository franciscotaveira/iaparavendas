...
const WarRoom: React.FC<WarRoomProps> = ({ isOpen, onClose, agents }) => {
  ...
  const handleSimulation = async () => {
    ...
    try {
      ...
      if (!keyToUse) {
         setSimulationLog(prev => [...prev, `[CRITICAL ERROR]: API KEY MISSING. ABORTING.`]);
      ...
      const prompt = `
        You are the Central Orchestrator of a Cyberpunk Corporate AI System.
        
        THE SCENARIO:
        The User (Operator) has initiated a WAR ROOM session with the following objective/crisis:
        "${objective}"
        
        YOUR TASK:
        Simulate a rapid-fire, high-stakes roundtable discussion between three specific agents:
        1. "VIPER" (Sales Lead): Aggressive, revenue-focused, hates losing deals.
        2. "NEO" (Tech Lead): Pragmatic, cynical, focused on feasibility and technical debt.
        3. "FORTRESS" (Security/Risk): Paranoid, compliant, risk-averse.
        
        FORMAT:
        Generate the dialogue script. Each agent speaks 1-2 times efficiently.
        Use strictly this format for each line:
        [AGENT_NAME]: The message content.
        
        At the end, provide a "FINAL_CONSENSUS" summary from the Orchestrator.
      `;

      // Streaming response for "live terminal" feel
      ...
      for await (const chunk of response) {
        ...
        // Split buffer by newlines to render as logs
        const lines = buffer.split('\n');
        // Keep the last incomplete line in buffer
        buffer = lines.pop() || "";
        
        setSimulationLog(prev => {
          // Filter out lines we've already rendered to avoid duplicates in strict mode
          // (Simplified logic for demo: just appending new complete lines)
      ...
      if (buffer.trim()) {
         setSimulationLog(prev => [...prev, buffer]);
      }
          ...
          <div className="w-full lg:w-1/3 border-r border-red-900/30 p-6 flex flex-col gap-6 bg-slate-900/50">
             ...
             <div>
               ...
               <input 
                 ...
                 className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-sm text-slate-300 focus:border-red-500 focus:outline-none placeholder-slate-600 font-mono"
               />
             </div>

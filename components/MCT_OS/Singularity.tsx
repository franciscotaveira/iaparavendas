...
const Singularity: React.FC<SingularityProps> = ({ isOpen, onClose, agents, currentDNA, onEvolveAgent }) => {
  ...
  const allAgents = agents ? Object.values(agents).flat() : [];
  ...
  const handleEvolution = async () => {
    ...
    try {
      ...
      if (!keyToUse) {
         setThoughtTrace(p => [...p, "CRITICAL: API KEY REQUIRED FOR SINGULARITY ACCESS."]);
         setStatus('error');
         return;
      }

      const ai = new GoogleGenAI({ apiKey: keyToUse });
      const currentPrompt = currentDNA[selectedAgentRole] || "Standard Protocol";

      // Trick to force "Thinking" visibility: We ask the model to output a specific thought block
      const prompt = `
        PROJECT OMEGA: AGENT EVOLUTION PROTOCOL
        TARGET AGENT: ${selectedAgentRole}
        CURRENT DNA: "${currentPrompt}"
        EVOLUTION GOAL: "${optimizationGoal}"

        TASK:
        1. Analyze the weaknesses of the Current DNA against the Evolution Goal.
        2. Design a new, "God-Tier" System Instruction (DNA) that is extremely capable, specific, and aggressive.
        3. The new DNA must include specific frameworks, behaviors, and output styles.

        OUTPUT FORMAT:
        You MUST stream your response.
        First, output your internal reasoning inside <thought_trace> tags. Be verbose. Show your analysis steps.
        Then, output the final DNA string inside <dna> tags.
      `;

      // Use Gemini 3.0 Pro for maximum reasoning capability
      const responseStream = await ai.models.generateContentStream({
        model: 'gemini-3-pro-preview', 
        contents: [{ role: 'user', parts: [{ text: prompt }] }],
        config: {
           thinkingConfig: { thinkingBudget: 4096 } // Give it brain power
        }
      });

      let buffer = "";
      let isInsideThinking = false;
      let isInsideDNA = false;
      let fullDNA = "";

      for await (const chunk of responseStream) {
        // Fix: .text is a property, not a method, and can be undefined
        const text = chunk.text || "";
        buffer += text;

        // Simple stream parsing simulation for visual effect
        // In reality, Gemini 3.0 Pro Thinking output is handled by the API, but we simulate the visual
        // feed by breaking down the text as it arrives.
        
        if (text.length > 0) {
           // Simulate "Matrix" decoding
           setThoughtTrace(prev => [...prev, ...text.split('\n').filter(Boolean)]);
        }
      }

      // Since parsing streaming XML tags is flaky, we just do a final extract
      const finalResponse = buffer;
      const dnaMatch = finalResponse.match(/<dna>([\s\S]*?)<\/dna>/i) || finalResponse.match(/DNA:([\s\S]*)/i);
      
      let extractedDNA = "";
      if (dnaMatch) {
        extractedDNA = dnaMatch[1].trim();
      } else {
        // Fallback: use the whole text if tags missing (Gemini sometimes disobeys formatting on complex tasks)
        extractedDNA = finalResponse.slice(Math.max(0, finalResponse.length - 500)); 
      }

      setGeneratedDNA(extractedDNA);
      setStatus('mutating');
      
      // Artificial delay for "Mutation" effect
      setTimeout(() => {
        onEvolveAgent(selectedAgentRole, extractedDNA);
        setStatus('complete');
      }, 1500);
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-1/3 border-r border-amber-500/20 p-6 flex flex-col gap-6 z-10 bg-black/50">
             ...
             <div className="relative">
                <label className="text-[10px] text-amber-600 font-mono uppercase mb-2 block flex items-center gap-2">
                   <Key size={10} /> Authentication Key (Root)
                </label>
                <input 
                  type="password" 
                  value={apiKey}
